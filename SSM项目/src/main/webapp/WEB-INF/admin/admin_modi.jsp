...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script language="javascript" type="text/javascript">
            //保存成功的提示消息
            function showResult() {
                var cbs = document.getElementsByName("roleIds");
                var checkNum = 0;
...
//                alert("选中数量=" + checkNum);
    ...
    <body>
        ...
        <div id="main">            
            ...
            <form action="${pageContext.request.contextPath}/admin/modify" method="post" class="main_form" id="main_form">
                ...
                <div class="text_info clearfix"><span>姓名：</span></div>
                    ...
                    <div class="input_info"><input type="text" readonly="readonly" class="readonly" value="${admin.adminCode}" name="adminCode"/></div>
                            ...
                            <ul>
                                <c:forEach items="${roles}" var="role">
                                    <li><input type="checkbox"
                                            <c:forEach items="${admin.roles}" var="aRole">
                                                <c:if test="${aRole.roleId == role.roleId}">
                                                    checked
                                                </c:if>
                                            </c:forEach>
                                               name="roleIds" value="${role.roleId}"/>${role.name}</li>
                                </c:forEach>
                            </ul>
                        </div>
                        <span class="required">*</span>
                        <div class="validate_msg_tiny error_msg" id="1" style="display: none">至少选择一个</div>
                    </div>
                    <div class="button_info clearfix">
                        <input type="button" value="保存" class="btn_save" onclick="showResult();" />
                        <input type="button" value="取消" class="btn_save" onclick="location.href='${pageContext.request.contextPath}/admin/findAdminByPage?pc=1'"/>
                    </div>
                </form>  
        </div>
        <!--主要区域结束-->
