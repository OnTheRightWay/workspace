    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
            ...
            function showResult() {
                ...
                var cbs = document.getElementsByName("moduleIds");
                var checkNum = 0;
...
//                alert("选中数量=" + checkNum);
    ...
    <body>
        ...
        <div id="main">           
            ...
            <form action="${pageContext.request.contextPath}/role/modify" method="post" class="main_form" id="main_form">
                        ...
                        <ul>
                            <c:forEach items="${modules}" var="module">
                                <li><input type="checkbox"
                                           <c:forEach items="${role.modules}" var="rModule">
                                               <c:if test="${module.moduleId == rModule.moduleId}">
                                                   checked
                                               </c:if>
                                           </c:forEach>
                                           name="moduleIds" value="${module.moduleId}"/>${module.name}</li>
                            </c:forEach>
                        </ul>
                    </div>
                    <span class="required">*</span>
                    <div class="validate_msg_tiny" id="1" style="display: none">至少选择一个权限</div>
                </div>
                <div class="button_info clearfix">
                    <input type="button" value="保存" class="btn_save" onclick="showResult()"/>
                    <input type="button" value="取消" class="btn_save" onclick="location.href='${pageContext.request.contextPath}/role/findRoleByPage?pc=1'"/>
                </div>
            </form> 
        </div>
        <!--主要区域结束-->
