...
<form action="<s:url action="postAction_addOrEditPost"/> " method="post">
	 ...
	 <tr>
	    ...
	    <td><select name="depId">
            ...
            <s:iterator value="#departments" var="dep">
                <option value="<s:property value="%{#dep.depId}"/>"
                        <s:if test="%{#dep.depId eq #post.department.depId}">
                        selected="selected"
                        </s:if>
                ><s:property value="%{#dep.depName}"/></option>
            </s:iterator>
		</select>
  </td>
	    <td>职务：</td>
